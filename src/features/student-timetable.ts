...
type StudentTimetableSubcommand = 'login' | 'logout' | 'status' | 'terms' | 'export';
...
export function resolveTerm(catalog: readonly AcademicTerm[], selector?: string): AcademicTerm {
  if (catalog.length === 0) throw new Error('No academic terms are available.');
  const selected = findAcademicTerm(catalog, selector);
  if (selected) return selected;
  if (!selector?.trim()) throw new Error('The current academic term could not be determined.');
  ...
  const allowedFlags =
    subcommand === 'export'
      ...
      : subcommand === 'logout'
        ? ['--plain']
        : [...commonFlags];
  const invalidFlag = [...options.flags].find(
    (flag) =>
      !allowedFlags.some((allowed) =>
      ...
      async (session) => {
        ...
        if (subcommand === 'terms') {
          ...
          for (const term of relevantTerms(catalog)) {
            stdout.write(
              `${term.current ? '*' : ' '} ${term.academicYear}:${term.semester}  ${term.academicYearLabel} ${displaySemesterLabel(term)}\n`,
            );
          }
          return 0;
        }
        ...
        if (timetable.unresolvedItems.length > 0) {
          stderr.write(
            `${fmt(trans.unresolvedPractice, { count: timetable.unresolvedItems.length })}\n`,
          );
        }
        return 0;
      },
