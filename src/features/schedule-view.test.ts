...
describe('peekTodayLines', () => {
  ...
  it("renders today's classes via the same rich timeline Schedule's hub uses, not the old flat list", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ termKey: '2026-1', weekOneMonday: '2026-09-14' }),
    );
    writeFileSync(
      join(dir, 'nbtca', 'timetable-2026-1.json'),
      JSON.stringify({
...
describe('peekWeekAheadInfo', () => {
  ...
  it('returns null when the term has not started yet (negative week)', () => {
    ...
    writeFileSync(
      join(dir, 'nbtca', 'current-term.json'),
      JSON.stringify({ termKey: '2026-1', weekOneMonday: '2099-01-05' }),
    );
    writeFileSync(
      join(dir, 'nbtca', 'timetable-2026-1.json'),
      JSON.stringify({
        term: { academicYear: '2099', semester: '1' },
        meetings: [],
        unresolvedItems: [],
        periods: [],
        calendarDays: [],
