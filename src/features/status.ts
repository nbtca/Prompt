...
export function renderServiceStatusTable(
  items: ServiceStatus[],
  ...
  const nameWidth = Math.max(
    ...items.map((i) => visualWidth(i.name)),
    visualWidth(trans.status.service),
  );
  ...
  for (const item of items) {
    ...
    const latencyCol =
      !item.pending && item.ok && item.latencyMs != null
