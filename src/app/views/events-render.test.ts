...
describe('renderEvents', () => {
  ...
  it.each([
    ...
    (language, mode, expected, rows) => {
      setLanguage(language);
      try {
        const state: EventsViewState = mode === 'loading' ? { mode } : { mode, heatmapBuckets: [] };
  ...
  it('hub mode shows the hub action list', () => {
    const hubField = new ListField({
      title: 'Events',
      options: [{ value: 'upcoming', label: 'Events' }],
    });
    const out = stripAnsi(
      renderEvents({ mode: 'hub', hubField, heatmapBuckets: [] }, new Date()).join('\n'),
    );
  ...
  it.each([
    ...
    [
      '活动',
      ['活动列表', '本周活动', '本月活动', '搜索活动', '往期活动', '过去十二个月活动热力图'],
    ],
  ...
  it('hub mode does not render the heatmap grid on a normal-size terminal, even when heatmapBuckets is populated', () => {
    ...
    const out = stripAnsi(
      renderEvents({ mode: 'hub', hubField, heatmapBuckets }, new Date('2026-07-15'), 19).join(
        '\n',
      ),
    );
    ...
    const recentEvents = [
      {
        ...
        location: 'TBD',
        description: '',
        startDate: new Date('2026-07-17T20:30:00'),
        recurring: true,
        uid: 'nwdc-1',
      },
    ];
...
describe('renderEvents — adaptive hub density', () => {
  ...
  it('shows only as many recent-activity events as fit, reserving room for the menu, on a normal terminal', () => {
    ...
    const out = stripAnsi(
      renderEvents({ mode: 'hub', hubField, recentEvents }, new Date('2026-07-15'), 15).join('\n'),
    );
  ...
  it('shows more recent-activity events on a tall terminal', () => {
    ...
    const out = stripAnsi(
      renderEvents({ mode: 'hub', hubField, recentEvents }, new Date('2026-07-15'), 45).join('\n'),
    );
...
describe('renderEvents — list/detail/search/error modes', () => {
  it('list mode shows the list field', () => {
    const listField = new ListField({
      title: 'Events',
      options: [{ value: '0', label: 'Hackathon' }],
    });
  ...
  it('detail mode shows the event title and the action list', () => {
    const detailField = new ListField({
      title: 'Hackathon',
      options: [{ value: 'export', label: 'Export .ics' }],
    });
    const out = stripAnsi(
      renderEvents(
        {
          mode: 'detail',
          detailField,
          detailTitle: 'Hackathon',
          detailMeta: '03-25  ·  Main Hall',
          detailDescription: 'Bring a laptop.',
        },
        new Date(),
      ).join('\n'),
    );
  ...
  it.each([
    ...
    'keeps the editable end of an events search query visible at twenty columns',
    (message, query) => {
      ...
      expect(text).toContain(Array.from(query).slice(-6).join(''));
      expect(lines.filter((line) => /[→>]/u.test(stripAnsi(line)))).toHaveLength(1);
    },
  );
  ...
  it('error mode shows the error message', () => {
    const out = stripAnsi(
      renderEvents({ mode: 'error', errorMessage: 'Broke' }, new Date()).join('\n'),
    );
